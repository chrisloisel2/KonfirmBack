...
const companyIntelligenceService_1 = require("../../services/companyIntelligenceService");
...
async function calculateRiskScore(dossierId) {
    ...
    const ppeSearches = dossier.recherches.filter((r) => r.type === 'PPE');
    const sanctionsSearches = dossier.recherches.filter((r) => r.type === 'SANCTIONS');
    const assetFreezeSearches = dossier.recherches.filter((r) => r.type === 'ASSET_FREEZE');
    // Score PPE
    let ppeScore = 0;
    for (const search of ppeSearches) {
        if (search.matches && Array.isArray(search.matches) && search.matches.length > 0) {
            const maxConfidence = Math.max(...search.matches.map((m) => m.confidence || 0));
    ...
    const missingDocs = requiredDocTypes.filter(type => !dossier.documents.some((doc) => doc.type === type));
    ...
    if (moyenPaiement?.type) {
        const mpType = moyenPaiement.type;
    ...
    try {
        const companyResult = await (0, companyIntelligenceService_1.checkCompanyIntelligence)({
            nom: client.nom,
...
router.post('/dossiers/:dossierId', auth_1.authenticateToken, auth_1.requireDossierAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { dossierId } = req.params;
    const { forceRecalculation } = scoringRequestSchema.parse(req.body);
    const userId = req.user.id;
    // Vérifier s'il existe déjà un scoring
    const existingScoring = await prisma_1.default.scoring.findUnique({
        where: { dossierId }
    });
    ...
    const scoring = existingScoring
        ? await prisma_1.default.scoring.update({
            where: { dossierId },
            data: {
                scoreTotal: scoringResult.scoreTotal,
                niveau: scoringResult.niveau,
                facteurs: scoringResult.facteurs,
        ...
        : await prisma_1.default.scoring.create({
            data: {
                dossierId,
                scoreTotal: scoringResult.scoreTotal,
                niveau: scoringResult.niveau,
    ...
    if (scoringResult.niveau === 'ELEVE' || scoringResult.niveau === 'CRITIQUE') {
        const existingException = await prisma_1.default.exception.findFirst({
            where: {
                dossierId,
                type: 'SCORING_ELEVE',
                status: { in: ['EN_ATTENTE', 'EN_COURS_TRAITEMENT'] }
...
router.get('/dossiers/:dossierId', auth_1.authenticateToken, auth_1.requireDossierAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    if (!scoring) {
        throw new errorHandler_1.ValidationError('Aucun scoring disponible pour ce dossier');
    }
    res.json({
        success: true,
        data: { scoring }
...
router.get('/dossiers/:dossierId/preview', auth_1.authenticateToken, auth_1.requireDossierAccess, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    const { dossierId } = req.params;
    const scoringResult = await calculateRiskScore(dossierId);
    res.json({
        success: true,
...
router.get('/stats', auth_1.authenticateToken, (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    const [totalScorings, distributionParNiveau, scoresMoyens, scoringsRecents] = await Promise.all([
        // Total des scorings
        prisma_1.default.scoring.count({ where: whereClause }),
        // Distribution par niveau de risque
        prisma_1.default.scoring.groupBy({
            by: ['niveau'],
            where: whereClause,
            _count: true
        }),
        ...
        prisma_1.default.scoring.count({
            where: {
                ...whereClause,
                calculatedAt: {
    ...
    const stats = {
        ...
        distribution: distributionParNiveau.reduce((acc, item) => {
            acc[item.niveau] = item._count;
            return acc;
        }, {}),
        averageScores: scoresMoyens.reduce((acc, item) => {
