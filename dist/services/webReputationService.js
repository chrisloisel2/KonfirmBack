...
exports.checkWebReputation = checkWebReputation;
...
function assessSeverity(kws) {
    ...
    if (isCriminal || (isJudicial && isFraud))
        return 'critical';
    if (isJudicial || (isFraud && isFinancial))
        return 'high';
    if (isFraud || isFinancial || isCounterfeit)
        return 'medium';
    return 'low';
}
...
function isRelevantPersonMention(fullName, text) {
...
// ─── 1. Google News RSS ───────────────────────────────────────────────────────
async function crawlGoogleNews(name) {
    const queries = [
...
async function crawlBODACC(nom, prenom) {
    const hits = [];
    ...
    try {
        const res = await axios_1.default.get(`${base}/annonces-pcl/records`, {
            params: {
        ...
        for (const r of (res.data?.results ?? [])) {
            ...
            if (!fullText.includes(nom.toLowerCase()))
                continue;
    ...
    try {
        const res = await axios_1.default.get(`${base}/annonces-commerciales/records`, {
...
// ─── 3. Bing News ─────────────────────────────────────────────────────────────
async function crawlBingNews(name) {
    const hits = [];
    ...
    try {
        const res = await axios_1.default.get('https://www.bing.com/news/search', {
...
// ─── 4. Qwant News ────────────────────────────────────────────────────────────
async function crawlQwantNews(name) {
    const hits = [];
    ...
    await Promise.allSettled(queries.map(async (q) => {
        try {
            const res = await axios_1.default.get('https://api.qwant.com/v3/search/news', {
...
async function crawlFrenchPress(name) {
    const hits = [];
    await Promise.allSettled(PRESS_SOURCES.map(async (src) => {
        try {
            const res = await axios_1.default.get(src.searchUrl(name), {
                timeout: 9000,
...
async function crawlPappers(nom, prenom) {
    const apiKey = process.env.PAPPERS_API_KEY;
    if (!apiKey)
        return [];
    try {
        const res = await axios_1.default.get('https://api.pappers.fr/v2/recherche-dirigeants', {
        ...
        for (const dirigeant of (res.data?.resultats ?? [])) {
            for (const c of (dirigeant.entreprises ?? [])) {
                ...
                if (!statut.includes('liquidation') && !statut.includes('cessation') && !statut.includes('dissolution'))
                    continue;
...
async function checkWebReputation(input) {
    ...
    try {
        ...
        const topStatus = (critical.length > 0 || high.length > 0) ? 'alert' : 'warning';
        ...
        (0, logger_1.logSystemEvent)({ action: 'ocr_error', component: 'webReputation', details: { error: msg }, severity: 'warning' });
        return { ...base, status: 'error', summary: 'Service indisponible', details: `Web Crawling: ${msg}`, matches: [] };
    }
}
//# sourceMappingURL=webReputationService.js.map