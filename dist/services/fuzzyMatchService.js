...
exports.computeFuzzyMatch = computeFuzzyMatch;
exports.screenNameAgainstList = screenNameAgainstList;
exports.matchFullName = matchFullName;
exports.matchDateOfBirth = matchDateOfBirth;
...
function tokenizedNameMatch(query, candidate) {
...
// ─── Composite scorer ────────────────────────────────────────────────────────
function computeFuzzyMatch(query, candidate, threshold = 0.72) {
    ...
    const matchType = score >= 0.97 ? 'EXACT' :
        score >= 0.88 ? 'STRONG' :
            score >= 0.80 ? 'PROBABLE' :
                score >= 0.70 ? 'POSSIBLE' :
                    score >= 0.55 ? 'WEAK' :
...
function screenNameAgainstList(query, list, threshold = 0.72) {
    return list
        .map((candidate, idx) => ({
        candidate,
...
function matchFullName(queryFirst, queryLast, candidateFirst, candidateLast, threshold = 0.72) {
    ...
    const composite = computeFuzzyMatch(`${queryFirst} ${queryLast}`, `${candidateFirst} ${candidateLast}`, threshold);
    composite.score = Math.max(composite.score, bestScore);
    composite.isMatch = composite.score >= threshold;
    const matchType = composite.score >= 0.97 ? 'EXACT' :
...
function matchDateOfBirth(queryDob, candidateDob) {
    if (!queryDob || !candidateDob)
    ...
    if (qn.length === 8 && cn.length === 8 && qn.substring(0, 6) === cn.substring(0, 6)) {
        ...
        if (diff <= 1)
            return { match: true, confidence: 0.85, note: '±1 jour' };
    }
    return { match: false, confidence: 0 };
}
//# sourceMappingURL=fuzzyMatchService.js.map