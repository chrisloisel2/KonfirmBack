...
exports.parseCSVToRecords = parseCSVToRecords;
exports.runBatchSearch = runBatchSearch;
exports.exportBatchResultsToCSV = exportBatchResultsToCSV;
exports.getBatchSearch = getBatchSearch;
exports.getUserBatchSearches = getUserBatchSearches;
...
async function processRecord(record, searchTypes, threshold) {
    ...
    try {
        ...
        const report = await (0, osintMegaService_1.runOsintQuick)(query);
        ...
        for (const src of report.sources) {
            if (src.matchCount === 0)
                continue;
            if (src.category === 'SANCTIONS')
                result.sanctionsHit = true;
            if (src.category === 'GEL_AVOIRS')
                result.gelAvoirsHit = true;
            if (src.category === 'INTERPOL')
                result.interpolHit = true;
            if (src.category === 'RISQUE_PAYS')
                result.paysRisqueHit = true;
            if (src.category === 'PRESSE')
                result.presseHit = true;
        }
        // Check Wikipedia for PPE
        const wikiSrc = report.sources.find(s => s.id === 'wikipedia');
        if (wikiSrc?.results.some(r => r.details?.isPEP))
            result.pepHit = true;
...
// ─── Run batch search ─────────────────────────────────────────────────────────
async function runBatchSearch(opts) {
    const start = Date.now();
    ...
    // Create DB record
    const batch = await prisma_1.default.batchSearch.create({
        data: {
            userId: opts.userId,
            name: batchName,
    ...
    for (const chunk of chunks) {
        const chunkResults = await Promise.all(chunk.map(record => processRecord(record, opts.searchTypes, threshold)));
        results.push(...chunkResults);
        // Save progress
        await prisma_1.default.batchSearch.update({
            where: { id: batch.id },
            data: { processedCount: results.length }
        });
        // Save results to DB
        for (const r of chunkResults) {
            await prisma_1.default.batchSearchResult.create({
                data: {
                    batchId: batch.id,
                    rowIndex: r.rowIndex,
                    inputData: { nom: r.nom, prenom: r.prenom, reference: r.reference },
                    hasHit: r.hasHit,
...
// ─── Export to CSV ────────────────────────────────────────────────────────────
function exportBatchResultsToCSV(results) {
    const headers = [
...
// ─── Get batch status ─────────────────────────────────────────────────────────
async function getBatchSearch(batchId, userId) {
    return prisma_1.default.batchSearch.findFirst({
...
async function getUserBatchSearches(userId) {
    return prisma_1.default.batchSearch.findMany({
        where: { userId },
...
//# sourceMappingURL=batchSearchService.js.map