...
import { normalize } from './fuzzyMatchService';
...
export interface AdvancedFilters {
	...
	revenus?: { min?: number; max?: number };
	patrimoine?: { min?: number; max?: number };
	pays?: string[];
	ville?: string;
...
function highlightText(text: string, query: string, maxLen = 200): string {
	if (!text) return '';
...
function computeRelevanceScore(text: string, query: string): number {
	...
	if (tl === ql) return 1.0;
	if (tl.startsWith(ql)) score += 0.9;
	else if (tl.includes(ql)) score += 0.7;

	// Token matches
	for (const token of tokens) {
		if (tl.startsWith(token)) score += 0.2;
		else if (tl.includes(token)) score += 0.1;
	}
...
export async function universalSearch(
	...
	const want = (type: EntityType) => searchAll || entityTypes!.includes(type);
...
async function searchClients(
	re: RegExp, query: string, filters: AdvancedFilters, dateFilter: any
	...
	return clients.map((c: any): SearchResult => {
		const fullName = `${c.prenom} ${c.nom}`;
		const score = Math.max(
			computeRelevanceScore(fullName, query),
			computeRelevanceScore(c.email || '', query),
			computeRelevanceScore(c.numeroIdentite || '', query),
			computeRelevanceScore(c.profession || '', query),
		);
		return {
			entityType: 'CLIENT',
			entityId: c.id,
...
async function searchDossiers(
	...
	return dossiers.map((d: any): SearchResult => ({
		entityType: 'DOSSIER',
		entityId: d.id,
...
async function searchDocuments(
	...
	return docs.map((doc: any): SearchResult => ({
		...
		entityId: doc.id,
		score: Math.max(
			computeRelevanceScore(doc.originalName, query),
			computeRelevanceScore(doc.ocrText || '', query) * 0.8,
		),
		highlight: {
...
async function searchExceptions(
	re: RegExp, query: string, filters: AdvancedFilters, dateFilter: any
): Promise<SearchResult[]> {
	const where: any = {
		AND: [
	...
	const exceptions = await (prisma as any).exception.findMany({
		where,
		take: 50,
		include: {
			dossier: {
	...
	return exceptions.map((ex: any): SearchResult => ({
...
async function searchTracfin(
	...
	const decls = await (prisma as any).tracfinDeclaration.findMany({
		where,
		take: 30,
		include: {
	...
	return decls.map((d: any): SearchResult => ({
		entityType: 'TRACFIN',
		entityId: d.id,
...
async function searchAuditLogs(
	...
	return logs.map((log: any): SearchResult => ({
		entityType: 'AUDIT',
		entityId: log.id,
...
async function searchRecherches(
	re: RegExp, query: string, filters: AdvancedFilters, dateFilter: any
): Promise<SearchResult[]> {
	const where: any = {
		AND: [
	...
	const recherches = await (prisma as any).recherche.findMany({
		where: {
			...where,
			dossier: { client: { OR: [
				{ nom:    { contains: query, mode: 'insensitive' } },
	...
	return recherches.map((r: any): SearchResult => ({
...
async function searchScoring(
	re: RegExp, query: string, filters: AdvancedFilters
): Promise<SearchResult[]> {
	const where: any = {
		AND: [
	...
	const scorings = await (prisma as any).scoring.findMany({
		where,
		take: 30,
	});

	return scorings.map((s: any): SearchResult => ({
		entityType: 'SCORING',
		entityId: s.id,
...
function buildDateFilter(filters: AdvancedFilters) {
...
function buildDossierAccessFilter(opts: UniversalSearchOptions) {
	if (!opts.userRole || ['REFERENT', 'RESPONSABLE', 'ADMIN'].includes(opts.userRole)) return {};
...
function emptyResponse(query: string, start: number): UniversalSearchResponse {
	return {
		results: [],
		total: 0,
...
export async function generateSuggestions(query: string): Promise<string[]> {
	...
	for (const d of dossiers) suggestions.add(d.numero);

	return Array.from(suggestions).slice(0, 8);
}

// ─── Timeline search (activité complète d'un client/dossier) ─────────────────
...
export async function getEntityTimeline(opts: {
	...
	if (clientId) {
		...
		dossierIds = dossiers.map((d: any) => d.id);
	} else if (dossierId) {
		dossierIds = [dossierId];
	}

	if (dossierIds.length === 0 && !opts.userId) return [];
	...
	if (dossierIds.length > 0) {
		...
		// TRACFIN
		const tracfins = await (prisma as any).tracfinDeclaration.findMany({
			where: {
				dossierId: { in: dossierIds },
				...(Object.keys(dateRange).length ? { createdAt: dateRange } : {}),
			},
...
export async function recordSearchHistory(
	...
	try {
		...
		logSystemEvent({ action: 'search_history_error', component: 'universalSearch', details: { error: String(e) }, severity: 'warn' });
	}
}
