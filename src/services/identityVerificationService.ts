...
import { checkWebReputation } from './webReputationService';
import { checkCompanyIntelligence } from './companyIntelligenceService';
import { checkGelAvoirsDGTresor } from './seuilsLcbFtService';
...
function checkDocument(input: IdentityInput): VerificationResult {
	const issues: string[] = [];
	...
	if (input.dateExpiration) {
		...
		if (expiry < today) {
			...
			if (daysLeft < 90) issues.push(`Document expire dans ${daysLeft} jours`);
		}
	}
	...
	if (input.dateNaissance) {
		...
		if (age < 0 || age > 120) issues.push('Date de naissance implausible');
	}

	const status: VerificationStatus = issues.length > 0
...
async function checkOpenSanctions(input: IdentityInput): Promise<VerificationResult> {
	...
	try {
		...
		const res = await axios.post('https://api.opensanctions.org/match/default', body, {
		...
		const sanctionMatches = results.filter((r: any) => r.datasets?.includes('sanctions'));
		const pepMatches = results.filter((r: any) => r.datasets?.some((d: string) => d.includes('pep')));
		...
		if (sanctionMatches.length > 0) {
			return {
				...
				details: `Score max: ${Math.round(topScore * 100)}% — Sources: ${sanctionMatches.map((r: any) => r.datasets.join(', ')).join(' | ')}`,
		...
		logSystemEvent({ action: 'ocr_error', component: 'opensanctions', details: { error: msg }, severity: 'warning' });
		return {
			...base,
			status: 'error',
...
async function checkOFAC(input: IdentityInput): Promise<VerificationResult> {
	const base: Omit<VerificationResult, 'status' | 'summary' | 'details' | 'matches'> = {
		id: 'ofac_sdn',
		source: 'OFAC SDN',
	...
	try {
		...
		const res = await axios.get(
			`https://sanctionssearch.ofac.treas.gov/SdnList.aspx?Program=ALL&Name=${fullName}&Type=individual`,
			{ timeout: 8000, headers: { 'User-Agent': 'Mozilla/5.0 (compatible; KonfirmApp/1.0)' } }
		);
		...
		if (rows > 0) {
			const names = $('#gvSearchResults tr:not(:first-child) td:first-child').map((_: number, el: any) => $(el).text().trim()).get();
			return {
				...base,
				status: 'alert',
...
async function checkInterpol(input: IdentityInput): Promise<VerificationResult> {
	const base: Omit<VerificationResult, 'status' | 'summary' | 'details' | 'matches'> = {
		id: 'interpol',
		source: 'Interpol',
	...
	try {
		...
		return {
			...
			details: notices.slice(0, 3).map((n: any) => `${n.forename ?? ''} ${n.name ?? ''} (${n.entity_id})`).join(' | '),
			matches: notices,
		};
	} catch (err) {
		const msg = err instanceof Error ? err.message : String(err);
...
async function checkWikipediaPEP(input: IdentityInput): Promise<VerificationResult> {
	const base: Omit<VerificationResult, 'status' | 'summary' | 'details' | 'matches'> = {
		id: 'wikipedia_pep',
		source: 'Wikipedia',
	...
	try {
		...
		const topHit = hits[0];
		const snippet: string = (topHit.snippet ?? '').toLowerCase();
		const title: string = topHit.title ?? '';
...
async function checkReputation(input: IdentityInput): Promise<VerificationResult> {
	const base: Omit<VerificationResult, 'status' | 'summary' | 'details' | 'matches'> = {
		id: 'reputation_web',
		source: 'DuckDuckGo',
	...
	try {
		...
		const abstract: string = (res.data?.Abstract ?? '').toLowerCase();
		const relatedTopics: any[] = res.data?.RelatedTopics ?? [];
		const allText = [abstract, ...relatedTopics.map((t: any) => (t.Text ?? '').toLowerCase())].join(' ');
...
async function checkEUSanctions(input: IdentityInput): Promise<VerificationResult> {
	const base: Omit<VerificationResult, 'status' | 'summary' | 'details' | 'matches'> = {
		id: 'eu_sanctions',
		source: 'UE — FSRB',
	...
	try {
		...
		const rss = await axios.get('https://webgate.ec.europa.eu/fsd/fsf/public/rss', {
			timeout: 8000,
			headers: { 'User-Agent': 'Mozilla/5.0 (compatible; KonfirmApp/1.0)' },
		});
		const tokenMatch = (rss.data as string).match(/token=([A-Za-z0-9+/=]+)/);
		...
		$('nameAlias').each((_: number, el: any) => {
			...
			if (
				(lastName.includes(nomLower) && firstName.includes(prenomLower)) ||
...
export async function verifyIdentity(input: IdentityInput): Promise<VerificationResult[]> {
	logSystemEvent({
		action: 'ocr_progress',
	...
	const [docResult, opensanctions, ofac, interpol, wikipedia, reputation, euSanctions, webReputation, companyIntel] =
		await Promise.allSettled([
			...
			checkWebReputation(input),
			checkCompanyIntelligence(input),
		]);

	const results = [docResult, opensanctions, ofac, interpol, wikipedia, reputation, euSanctions, webReputation, companyIntel].map(r =>
 ...
 */
export async function verifyIdentityLCBFT(input: IdentityInput): Promise<VerificationResult[]> {
	logSystemEvent({
	...
	const [
		...
		checkGelAvoirsDGTresor(`${input.prenom} ${input.nom}`)
	]);
...
function getAge(frDate: string): number {
