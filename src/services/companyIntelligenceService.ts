...
export interface CompanyRecord {
  ...
  role?: string;            // gérant, président, associé…
  dateCreation?: string;
  dateFermeture?: string;
  lifespanMonths?: number;  // null si encore active
  activity?: string;        // code NAF
  activityLabel?: string;   // libellé
  address?: string;
  city?: string;
  legalForm?: string;
  isHolding: boolean;
  isSoleTrader: boolean;    // entrepreneur individuel (EI / micro-entreprise)
  source: string;
}
...
function monthsBetween(d1: string, d2: string | null | undefined): number | undefined {
...
async function queryINSEE(nom: string, prenom: string): Promise<CompanyRecord[]> {
  ...
  while (page <= totalPages && page <= 5) {
    try {
      ...
      for (const company of (data?.results ?? [])) {
        ...
        const isDirigeant = (company.dirigeants ?? []).some((d: any) =>
          d.nom?.toLowerCase().includes(nomLower) &&
        ...
        if (!isDirigeant && !isOwnEI) continue;

        const role = (company.dirigeants ?? []).find((d: any) =>
          d.nom?.toLowerCase().includes(nomLower)
        )?.qualite ?? (isOwnEI ? 'Entrepreneur individuel' : 'Dirigeant');
...
async function queryBODACCBySiren(siren: string): Promise<'liquidation' | 'redressement' | null> {
  try {
    const res = await axios.get(
      'https://bodacc-datadila.opendatasoft.com/api/explore/v2.1/catalog/datasets/annonces-pcl/records',
      {
    ...
    if (type.includes('redressement')) return 'redressement';
    return null;
  } catch {
    return null;
  }
...
async function querySocieteCom(nom: string, prenom: string): Promise<CompanyRecord[]> {
  const results: CompanyRecord[] = [];

  try {
    const res = await axios.get(
      `https://www.societe.com/cgi-bin/search?champs=${encodeURIComponent(`${prenom} ${nom}`)}`,
      {
        timeout: 10000,
        headers: {
...
function detectPatterns(companies: CompanyRecord[]): CompanyPattern[] {
  const patterns: CompanyPattern[] = [];
  if (companies.length === 0) return patterns;
  ...
  const nafSectors = [...new Set(all.map(c => c.activity?.slice(0, 4)).filter((s): s is string => !!s))];
  ...
  const addressMap = new Map<string, CompanyRecord[]>();
  for (const c of all) {
...
export async function checkCompanyIntelligence(input: IdentityInput): Promise<VerificationResult> {
  ...
  try {
    ...
    companies = companies.filter(c => {
      if (!c.siren) return true;
      if (seenSiren.has(c.siren)) return false;
    ...
    await Promise.allSettled(
      companies
        ...
        .map(async (c) => {
          ...
          if (bodaccStatus === 'liquidation') c.status = 'liquidation';
          else if (bodaccStatus === 'redressement') c.status = 'closed';
        })
    );
    ...
    const topStatus: VerificationStatus = critical.length > 0 ? 'alert' : high.length > 0 ? 'alert' : 'warning';

    const detailLines = patterns.slice(0, 4).map(p =>
      `⚠ [${p.severity.toUpperCase()}] ${p.label}: ${p.description}`
    );

    detailLines.push(
      '',
      `Entreprises (${companies.length} total, ${active} actives, ${closed} fermées):`,
      ...companies.slice(0, 8).map(c =>
        `  • ${c.name} (${c.status === 'active' ? '✓ active' : '✗ fermée'}${c.dateCreation ? ` — créée ${c.dateCreation}` : ''}${c.lifespanMonths ? ` — ${c.lifespanMonths}m` : ''}) [${c.activityLabel ?? c.activity}]`
      )
    );
    ...
    logSystemEvent({ action: 'ocr_error', component: 'companyIntelligence', details: { error: msg }, severity: 'warning' });
